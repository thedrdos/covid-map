...
state_data  = {}
county_data = {}
first_date =  int((pd.Timestamp.now()-pd.DateOffset(months=4)).asm8)
for l in ext_datafiles['location_to_filename']:
   if 'US' in l:
       with gzip.GzipFile(ext_datafiles['path']+ext_datafiles['location_to_filename'][l]+'.json.gz', 'r') as fin:
           datafile = json.loads(fin.read().decode('utf-8'))
           ...
           data = datafile['data']
           try:
               ind0 = next(n for n,x in enumerate(datafile['data']['date']) if x>=first_date)
           except:
               ind0 = 0
           if datafile['type']=='state':
               state_data[l] = {}
               for k in keys_keep:
                   state_data[l][k] = data[k][ind0:]
           if datafile['type']=='county':
               county_data[l] = {}
               for k in keys_keep:
                   county_data[l][k] = data[k][ind0:]
...
county_map = {}
for l in ext_datafiles['location_to_filename']:
   if 'US' in l and l.count(',')<2:
       with gzip.GzipFile(ext_datafiles['path']+ext_datafiles['location_to_filename'][l]+'_map.json.gz', 'r') as fin:
           datafile = json.loads(fin.read().decode('utf-8'))
           data = datafile['data']
           if datafile['type']=='country':
               state_map[l] = {}
               for k in keys_keep:
                   state_map[l][k] = data[k]
           if datafile['type']=='state':
               county_map[l] = {}
               for k in keys_keep:
                   county_map[l][k] = data[k]
nan_code = datafile['nan_code']
# Create source data structure and initialize state map
init_state = 'Florida, US'
source_state_data = ColumnDataSource(state_data[init_state])
source_county_data = ColumnDataSource(state_data[init_state])
source_county_map = ColumnDataSource(county_map[init_state])
# Erase the underlying data to reduce the html filesize (will be loaded upon user tap feedback)
source_state_data.data  = {k:[] for k in source_state_data.data}
source_county_data.data = {k:[] for k in source_county_data.data}
source_county_map.data  = {k:[] for k in source_county_map.data}
...
yleft = {
    'label':  "Bar Graph: # per Million",
    'source': source_county_data,
    'x': 'date',
    'y': ['positiveActivePerMil',
          'deathPerMil',
          'recoveredPerMil'],
    'color': ["blue","black","orange"],
    'legend_label': [
...
p_county_map = figure(
    ...
    tooltips=[
        ...
        ('Recovered Cases (est)',"@recovered{0,0.}"),
...
#     line_width=1,
#     )
patches_counties = p_county_map.multi_polygons(
    xs='x', ys='y', source=source_county_map,
...
with open(javascript_path+'callback_county_map.js','r') as f:
    callback_county_map = f.read()
callbacktap = CustomJS(args={
